...
def parse_xml_blocks(ingest: IngestionResult) -> list[Block]:
    ...
    for elem in root.iter():
        # Tag with text or attributes
        text = (elem.text or "").strip()
        attrs_str = " ".join(f'{k}="{v}"' for k, v in elem.attrib.items())
        if not text and not attrs_str:
            continue

        tag_name = elem.tag.split("}")[-1] if "}" in elem.tag else elem.tag
        content_for_search = (
            text if text else (list(elem.attrib.values())[0] if elem.attrib else tag_name)
        )
        start_c, end_c = find_span(content_for_search)
        span = clean_char_to_byte_span(ingest, start_c, end_c)

        if text and attrs_str:
            node_text = f"{text} ({attrs_str})"
        elif attrs_str:
            node_text = attrs_str
        else:
            node_text = text
