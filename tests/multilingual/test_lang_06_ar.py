"""Honest multilingual test suite for Arabic (ar).
...
CORPUS_FILE = REPO_ROOT / "examples/eval/multilingual" / "07_ar_kalila_wa_dimna.txt"
...
ATOM_CASES_AR = [
    (
        "أصدر خادم الإنتاج 10.0.4.12 تنبيهاً بعد أن بلغ استخدام المعالج 45 ms، والفريق يتابع الأمر ضمن CVE-2026-10432.",
        "CVE-2026-10432",
        "10.0.4.12",
        "45 ms",
    ),
    (
        "ربط المهندسون ارتفاع زمن الاستجابة 120 ms على المضيف 10.0.9.31 بالثغرة CVE-2026-20981.",
        "CVE-2026-20981",
        "10.0.9.31",
        "120 ms",
    ),
    (
        "تم نشر تصحيح للثغرة CVE-2026-31207 على 10.1.2.44، مما أعاد الإنتاجية إلى ما فوق 78 ms.",
        "CVE-2026-31207",
        "10.1.2.44",
        "78 ms",
    ),
    (
        "أصدر خادم الإنتاج 10.1.7.19 تنبيهاً بعد أن بلغ استخدام المعالج 212 ms، والفريق يتابع الأمر ضمن CVE-2026-40556.",
        "CVE-2026-40556",
        "10.1.7.19",
        "212 ms",
    ),
    (
        "ربط المهندسون ارتفاع زمن الاستجابة 33 ms على المضيف 10.2.5.63 بالثغرة CVE-2026-50871.",
        "CVE-2026-50871",
        "10.2.5.63",
        "33 ms",
    ),
    (
        "تم نشر تصحيح للثغرة CVE-2026-61190 على 10.2.8.27، مما أعاد الإنتاجية إلى ما فوق 156 ms.",
        "CVE-2026-61190",
        "10.2.8.27",
        "156 ms",
    ),
...
def test_ar_negation_detection(text: str):
    ingest = ingest_bytes(text.encode("utf-8"))
    atoms = extract_atoms(ingest)
...
@pytest.mark.parametrize("text", SPAN_CASES_AR)
def test_ar_utf8_byte_span_roundtrip(text: str):
...
def test_ar_end_to_end_real_corpus_slice():
